...
function Opticode({ user }) {
    ...
    const lineNums = () => {
        ...
        return lines.map((line, index) => (
            <p key={index} className='opticode-line-num'>{index + 1}</p>
        ));
    };
    ...
    const runCode = async (action) => {
        const userCode = document.getElementById('opticode-code-input').value;
    ...
    if (submitted) {
        <div className='opticode-cont'>
            <h1>Waiting for opponent...</h1>
        </div>
    } else if (matchFound.current && currSocket !== null && problem.length !== 0) {
        return (
            <div className='opticode-cont'>
                <div className='opticode-header'>
                    <div className='opticode-timer-cont'>
                        <h1>{formatTime(time)}</h1>
                    </div>
                    <div className='opticode-opponent-info-cont'>
                        <h3>Opponent: {opponent}</h3>
                        <p>Max Cases Passed: {maxCases}/10</p>
                    </div>
                </div>
                <div className='opticode-ide-cont'>
                    <div className='opticode-ls'>
                        <div className='opticode-line-nums' ref={nums}>
                            {lineNums()}
                        </div>
                        <div className='opticode-code-input-cont'>
                            <textarea value={code} onKeyDown={handleKeyPress} onPaste={/*e => e.preventDefault()*/null} ref={codeInput} onScroll={scrollNums} onChange={e => setCode(e.target.value)} id='opticode-code-input' className='opticode-code-input'>
                            
                            </textarea>
                        </div>
                    </div>
                    <div className='opticode-rs'>
                        <div className='opticode-problem-cont'>
                            <h1 className='opticode-problem-title'>{problem[1]}</h1>
                            <textarea readOnly value={problemText} className='opticode-problem'></textarea>
                        </div>
                        <div className='opticode-console-cont'>
                            <textarea ref={codeConsole} value={consoleText} readOnly className='opticode-console'></textarea>
                        </div>
                    </div>
                </div>
                <div className='opticode-footer'>
                    <div className='opticode-code-actions'>
                        <button onClick={() => setCode("def " + problem[5] + ":")}>Reset Code</button>
                        <button>Submit</button>
                    </div>
                    <div className='opticode-run-actions'>
